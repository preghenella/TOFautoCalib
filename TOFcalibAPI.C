...
Bool_t
runCalib(const Char_t *filename, const Char_t *paramfilename = NULL, Bool_t useTimeZeroTOF = kFALSE, Bool_t useLHCClockPhase = kFALSE, Bool_t runFix = kTRUE, Int_t nSteps = 2, Int_t evMax = kMaxInt, Int_t minEntries = 100)
{
  ...
  setvbuf(stdout, NULL, _IOLBF, 0);
    ...
    if (runFix) {
        ...
        fitCalibHistos(10., 10., minEntries, kTRUE);
    ...
    for (Int_t istep = 0; istep < nSteps; istep++) {
        ...
        fitCalibHistos(3., 2., minEntries, kFALSE);
...
checkCalib(const Char_t *filename, const Char_t *paramfilename = NULL, Bool_t useTimeZeroTOF = kFALSE, Bool_t useLHCClockPhase = kFALSE, Bool_t runExtended = kTRUE, Int_t evMax = kMaxInt)
{
    ...
    if (runExtended) {
        ...
        deltatMax = +48800.;
        fillCalibHistos(useTimeZeroTOF, useLHCClockPhase);
        writeCalibHistos("checkcalib.extended-50ns.root");
        deltatMin = -24400.;
        deltatMax = +24400.;
        fillCalibHistos(useTimeZeroTOF, useLHCClockPhase);
...
fillPerfHistos(Bool_t useTimeZeroTOF = kFALSE, Bool_t useLHCClockPhase = kFALSE)
{
    ...
    if (!hPerfHistoDeltaT)
        hPerfHistoDeltaT = new TH2F("hPerfHistoDeltaT", "", 1000, 0., 20., 20000, -244000., 244000.);
    else
        hPerfHistoDeltaT->Reset();
    
    if (!hPerfHistoBeta)
        hPerfHistoBeta = new TH2F("hPerfHistoBeta", "", 1000, 0., 20., 2200, 0., 1.1);
    else
...
problematicChannels(const Char_t *filename, Float_t nsigma = 5., Int_t minHits = 100, Double_t manualCut = 0., Int_t startRun = 0, Int_t endRun = AliCDBRunRange::Infinity(), const Char_t *dbString = "local://$HOME/OCDB")
{
    ...
    Double_t cutLimit = dgaus->GetParameter(1) - 5. * dgaus->GetParameter(2);
    Double_t cutLimit2 = dgaus->GetParameter(1) - 5. * dgaus->GetParameter(3);
    ...
    for (Int_t ich = 0; ich < 157248; ich++) {
	    ...
	    hbad[nprobch] = hpy;
	    hbad[nprobch]->SetName(Form("hbad_%d", ich));
	    hbad[nprobch]->SetTitle(Form("fraction = %f", fraction_ch));
            nprobch++;
	    printf("bad %d: %d entries | %f fraction \n", ich, entries_ch, fraction_ch);
...
problematicFEAs(const Char_t *filename, Float_t nsigma = 3.)
{
    ...
    printf("found %f hits in +- %f sigma\n", fraction_all, sigma);
...
updateParOfflineEntry(const Char_t *paramfilename, const Char_t *ocdbfilename, Int_t startrun, Int_t endrun = AliCDBRunRange::Infinity(), const Char_t *dbString = "alien://?folder=/alice/cern.ch/user/r/rpreghen/OCDB")
{
    ...
    if (TString(ocdbfilename).BeginsWith("alien:"))
      TGrid::Connect("alien");
...
Bool_t
calibAbsolute(const Char_t *paramfilename, const Char_t *ocdbfilename)
{
    ...
    TH1D *hshiftch = (TH1D *)paramfile->Get("hCalibParam_channel_mean");
    if (!hshiftch) {
        return kTRUE;
    }
    TFile *ocdbfile = TFile::Open(ocdbfilename);
    ...
    TH1D *habsparams = (TH1D *)hshiftparams->Clone("hChannelParam_mean");
    ...
    for (Int_t idx = 0; idx < 157248; idx++) {
        currentparchannel = (AliTOFChannelOffline *)currentpararray->At(idx);
	currentslewpar = currentparchannel->GetSlewPar(0);
	habsparams->AddBinContent(idx + 1, 1.e3 * currentslewpar);
    }
    
    TFile *fout = TFile::Open("calibAbsolute.root", "RECREATE");
    habsparams->Write();
    hshiftch->Write();
    fout->Close();
...
Bool_t
calibDiff(const Char_t *paramfilename1, const Char_t *paramfilename2)
{

    TFile *paramfile1 = TFile::Open(paramfilename1);
    if (!paramfile1 || !paramfile1->IsOpen()) {
        return kTRUE;
    }
    TH1D *hshiftparams1 = (TH1D *)paramfile1->Get("hChannelParam_mean");
    if (!hshiftparams1) {
        return kTRUE;
    }
    TH1D *hshiftch1 = (TH1D *)paramfile1->Get("hCalibParam_channel_mean");
    if (!hshiftch1) {
        return kTRUE;
    }


    TFile *paramfile2 = TFile::Open(paramfilename2);
    if (!paramfile2 || !paramfile2->IsOpen()) {
        return kTRUE;
    }
    TH1D *hshiftparams2 = (TH1D *)paramfile2->Get("hChannelParam_mean");
    if (!hshiftparams2) {
        return kTRUE;
    }
    TH1D *hshiftch2 = (TH1D *)paramfile2->Get("hCalibParam_channel_mean");
    if (!hshiftch2) {
        return kTRUE;
    }

    TH1D *hshiftparamsd = (TH1D *)hshiftparams1->Clone("hshiftparamsd");
    hshiftparamsd->Add(hshiftparams2, -1.);
    
    TH2 *hChannelShiftMap = new TH2F("hChannelShiftMap", "", 18*48, 0., 18., 91*2, 0., 91.);
    TH1 *hDiff = new TH1F("hDiff", "", 4880, -2440., 2400.);
    Float_t hitmap[2];
    for (Int_t ich = 0; ich < 157248; ich++) {
      if (hshiftch1->GetBinError(ich + 1) <= 0.) continue;
      if (hshiftch2->GetBinError(ich + 1) <= 0.) continue;
      getHitMapXY(kChannel, ich, hitmap);
      hChannelShiftMap->Fill(hitmap[0], hitmap[1], TMath::Abs(hshiftparamsd->GetBinContent(ich + 1)));
      hDiff->Fill(hshiftparamsd->GetBinContent(ich + 1));
   }

    TFile *fout = TFile::Open("calibDiff.root", "RECREATE");
    hChannelShiftMap->Write();
    hDiff->Write();
    fout->Close();
